...
export const comptrollerFormatters: { [functionName: string]: TransactionFormatter } = {
  ...
  '_setCompSpeeds(address[],uint256[],uint256[])': async (
    ...
    for (let i = 0; i < addresses.length; i++) {
      ...
      const baseText = `\n\nSet CompSpeed for [${symbol}](https://${platform}/address/${currentAddress})`
      ...
      const changeInSpeedsText = (
        ...
        return `${type} speed of [${symbol}](${tokenUrl}) to ${newFormattedValue} COMP/block which was previously ${prevFormattedValue} (${change}).`
      }
