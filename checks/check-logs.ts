...
export const checkLogs: ProposalCheck = {
  ...
  async checkProposal(proposal, sim, deps) {
    ...
    const simLogs = sim.transaction.transaction_info.call_trace.logs
    ...
    bridgedSimulations.forEach((b) => {
      const bridgedLogs: Log[] =
        b.sim?.simulation_results
          .map((sr) => {
            return (
              sr.transaction?.transaction_info.call_trace.logs?.filter((log) => {
                if (
...
function createLogResult(simLogs: Log[], deps: ProposalData, tenderlyContracts: TenderlyContract[]) {
  const info: string[] = []
